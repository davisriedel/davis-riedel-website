...
export async function GET() {
  const filePath = path.join(process.cwd(), "src", 'app', "api", "admin", "config.yml", 'config.yml.hbs');
  ...
  if (process.env.NEXT_PUBLIC_VERCEL_ENV == "production") {
    ...
    const url = process.env.NEXT_PUBLIC_VERCEL_BRANCH_URL ?? "undefined (DECAP_BASE_URL)";
    if (url) decap_url = url;
  }
